...
$token_id = $_GET['token'];
        $query51="SELECT * from token  where id='$token_id' "; 
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/video_call_style.css?<?php echo time(); ?>" />
...
<body class='video'>
    ...
    <main>
        ...
        <h1 id="site-title">Video Call</h1>
		
        <div id="join-wrapper">
		 <!--   <input id="username" type="text" name="name" placeholder="Enter your name..." /> -->
			<input id="username" type="text" name="name" placeholder="Enter your name..." />
        ...
        <div id="footer">
            <div class="icon-wrapper">
                <img class="control-icon" id="camera-btn" src="imgs/video.svg" />
                <span>Cam</span>
            </div>

            <div class="icon-wrapper">
                <img class="control-icon" id="mic-btn" src="imgs/microphone.svg" />
                <span>Mic</span>
            </div>

            <div class="icon-wrapper">
                <img class="control-icon" id="leave-btn" src="imgs/leave.png" />
                <span>Leave</span>
            </div>
...
let joinStreams = async () => {
    ...
    client.on("volume-indicator", function(evt){
        for (let i = 0; evt.length > i; i++){
            ...
            if(volume > 0){
                document.getElementById(`volume-${speaker}`).src = 'imgs/volume-on.svg'
            }else{
                document.getElementById(`volume-${speaker}`).src = 'imgs/volume-off.svg'
            }
...
let handleUserJoined = async (user, mediaType) => {
    ...
    if (mediaType === 'video'){
        ...
        player = `<div class="video-containers" id="video-wrapper-${user.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${user.uid}" src="imgs/volume-on.svg" /> ${user.uid}</p>
